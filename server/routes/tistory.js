...
const TISTORY_REDIRECT_URI =
  process.env.TISTORY_REDIRECT_URI || 'http://localhost:3001/api/tistory/callback';
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    // For this MVP, we'll redirect back to the frontend with the token in the URL.
    // SECURITY NOTE: In production, use HttpOnly cookies or a postMessage flow.
    const clientUrl = process.env.CLIENT_URL || 'http://localhost:5173';
    res.redirect(`${clientUrl}?tistory_token=${accessToken}`);
  } catch (error) {
