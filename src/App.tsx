    ...
    <ErrorBoundary>
      <div className="min-h-screen bg-[#F8FAFC]">
                ...
                {history.length === 0 ? (
                  ...
                  history.map((item) => (
                    <div
                      ...
                      className="p-4 bg-slate-50 rounded-xl border border-slate-200 hover:border-indigo-500 cursor-pointer transition-all group"
                    >
                      <div className="text-xs text-indigo-600 font-bold mb-1">{item.date}</div>
                      <div className="font-bold text-slate-800 text-sm line-clamp-2 mb-2">
                        {item.topic}
                      </div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-12">
          ...
          {/* Step Views */}
          <div className="animate-in fade-in duration-500">{renderStepContent()}</div>
        </main>
      </div>
    </ErrorBoundary>
  );
