...
export const AuthGate: React.FC<AuthGateProps> = ({ onAuthenticated }) => {
  ...
  useEffect(() => {
    ...
    if (isCodeValid && isKeyValid) {
      ...
      // Access code passed, but need API Key (or just let them in to set it later)
      // Current design: Double Gate -> Code then Key
      setStep(1); 
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (step === 0) {
      ...
      if (inputValue.startsWith('AIza') && inputValue.length > 20) {
        ...
        setError('유효하지 않은 API Key 형식입니다. (AIza...)');
      }
    }
  };
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-slate-200 text-center animate-in fade-in zoom-in duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            ...
            className="w-full p-4 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
            autoFocus
          />
