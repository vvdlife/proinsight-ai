      ...
      <div className="p-2 overflow-x-auto custom-scrollbar">
        <div className="grid grid-rows-2 grid-flow-col gap-4 p-2 min-w-max">
          {marketData.map((item) => (
            <div
              key={item.symbol}
              className="flex flex-col min-w-[140px] px-4 py-3 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow relative group"
            >
              ...
              <div
                className={`text-xs font-bold flex items-center gap-1 ${item.change >= 0 ? 'text-red-500' : 'text-blue-500'}`}
              >
                <span>
              ...
              <div className="absolute inset-0 bg-white/90 hidden group-hover:flex items-center justify-center rounded-xl opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={(e) => {
                    ...
                    if (item.symbol === 'GC=F')
                      url =
                        'https://finance.naver.com/marketindex/worldGoldDetail.naver?marketindexCd=CMDT_GC';
                    else if (item.symbol === '005930.KS')
                      url = 'https://finance.naver.com/item/main.naver?code=005930';
                    else if (item.symbol === '035420.KS')
                      url = 'https://finance.naver.com/item/main.naver?code=035420';
                    else if (item.symbol === '^KS11')
                      url = 'https://finance.naver.com/sise/sise_index.naver?code=KOSPI';
                    else if (item.symbol === '^GSPC')
                      url = 'https://finance.naver.com/world/sise.naver?symbol=SPI@SPX';
                    else if (item.symbol === 'KRW=X')
                      url =
