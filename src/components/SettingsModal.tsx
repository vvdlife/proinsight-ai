...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleLogout = () => {
    if (confirm('앱을 잠그시겠습니까? 다시 사용하려면 API Key를 입력해야 합니다.')) {
      sessionStorage.removeItem('proinsight_api_key');
      // Reload to trigger AuthGate
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 space-y-6">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">API Key 상태</label>
            <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
              <div className="w-2 h-2 rounded-full bg-green-500"></div>
              <span className="text-slate-600 text-sm font-mono flex-1 truncate">{apiKey || '설정됨'}</span>
            </div>
            <p className="text-xs text-slate-400 mt-2">API Key는 브라우저 세션에 안전하게 저장됩니다.</p>
          </div>
