...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = React.memo(
  ({ content, font = BlogFont.PRETENDARD }) => {
    // Font Class Mapping
    const fontClass = useMemo(() => {
      switch (font) {
        case BlogFont.PRETENDARD:
          return 'font-pretendard';
        case BlogFont.NOTO_SERIF:
          return 'font-noto-serif';
        case BlogFont.NANUM_GOTHIC:
          return 'font-nanum-gothic';
        case BlogFont.RIDIBATANG:
          return 'font-ridibatang';
        case BlogFont.NANUM_PEN:
          return 'font-nanum-pen';
        default:
          return 'font-pretendard';
      }
    }, [font]);
    ...
    const renderedContent = useMemo(() => {
      ...
      while (i < lines.length) {
        ...
        if (trimmedLine.startsWith('```mermaid') || trimmedLine.startsWith('``` mermaid')) {
          ...
          elements.push(<MermaidDiagram key={`mermaid-${i}`} chart={mermaidLines.join('\n')} />);
        ...
        if (line.trim().startsWith('|')) {
          ...
          if (tableRows.length >= 2) {
            ...
            elements.push(
              <div
                key={`table-${i}`}
                className={`my-10 overflow-x-auto rounded-xl border border-slate-200 shadow-sm ${fontClass}`}
              >
                <table className="w-full min-w-[600px] text-left border-collapse bg-white">
                  <thead>
                    <tr className="bg-slate-100/80 border-b border-slate-200">
                      {headerRow.map((cell, idx) => (
                        <th
                          key={idx}
                          className="px-6 py-4 font-bold text-slate-700 text-xs uppercase tracking-wider"
                        >
                          {parseInline(cell)}
                        </th>
                      ))}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {bodyRows.map((row, rIdx) => (
                      <tr
                        key={rIdx}
                        className="even:bg-slate-50 hover:bg-indigo-50/40 transition-colors duration-150 ease-in-out"
                      >
                        {row.map((cell, cIdx) => (
                          <td
                            key={cIdx}
                            className="px-6 py-4 text-slate-600 text-[15px] leading-relaxed border-r border-slate-50 last:border-r-0 min-w-[150px] break-words"
                          >
                            {parseInline(cell)}
                          </td>
                        ))}
        ...
        if (line.startsWith('### ')) {
          ...
          elements.push(
            <h3
              key={i}
              className={`text-xl font-bold text-slate-800 mt-10 mb-4 flex items-center ${fontClass}`}
            >
          ...
          elements.push(
            <h2
              key={i}
              className={`text-2xl font-bold text-slate-900 mt-16 mb-6 pb-3 border-b-2 border-slate-100 ${fontClass}`}
            >
              {parseInline(line.replace('## ', ''))}
            </h2>,
          );
          ...
          elements.push(
            <h1
              key={i}
              className={`text-4xl font-extrabold text-slate-900 mb-10 tracking-tight leading-tight ${fontClass}`}
            >
              {parseInline(line.replace('# ', ''))}
            </h1>,
          );
          ...
          elements.push(
            <li
              key={i}
              className={`ml-4 flex items-start gap-3 mb-3 text-[17px] text-slate-700 leading-relaxed ${fontClass}`}
            >
          ...
          elements.push(
            <blockquote
              key={i}
              className="font-serif-kr text-xl italic text-slate-700 my-8 pl-6 border-l-4 border-indigo-300 bg-slate-50/80 py-6 pr-6 rounded-r-xl leading-relaxed shadow-sm"
            >
          ...
          elements.push(
            <p
              key={i}
              className={`text-[17px] text-slate-700 leading-[1.8] mb-6 font-normal tracking-normal ${fontClass}`}
            >
              {parseInline(line)}
            </p>,
          );
    ...
    return <div className={`prose prose-slate max-w-none ${fontClass}`}>{renderedContent}</div>;
  },
);
  ...
  return parts.map((part, i) => {
    ...
    if (typeof part === 'string' && part.startsWith('**') && part.endsWith('**')) {
      return (
        <strong
          key={`b-${i}`}
          className="font-bold text-slate-900 bg-yellow-100/80 px-1 rounded-sm shadow-sm decoration-clone"
        >
          {part.slice(2, -2)}
        </strong>
      );
