...
export const useBlogGenerator = () => {
  ...
  const generateOutlineWrapper = useCallback(
    async (
      topic: string,
      ...
      setLoading({
        isLoading: true,
        message: 'Gemini가 자료를 분석하고 개요를 작성하고 있습니다...',
        progress: 30,
      });
      try {
  ...
  const generatePostWrapper = useCallback(
    async (
      outline: OutlineData,
      tone: BlogTone,
      ...
      try {
        ...
        setLoading({
          isLoading: true,
          message: `Step 1/3: ${language === 'en' ? 'Translating' : 'Writing'} content...`,
          progress: 30,
        });
        // API Signature: (outline, tone, files, urls, memo, language, topic, modelId)
        const post = await apiGeneratePost(
          outline,
          tone,
          files,
          urls,
        ...
        if (!skipImage) {
          setLoading({
            isLoading: true,
            message: 'Step 2/3: AI가 썸네일 이미지를 생성 중입니다...',
            progress: 60,
          });
          try {
  ...
  const generateTranslationWrapper = useCallback(
    async (originalPost: BlogPost, model: ModelType): Promise<BlogPost> => {
  ...
  // Wrapper for Social
  const generateSocialWrapper = useCallback(
    async (title: string, summary: string, imageStyle: ImageStyle): Promise<SocialPost[]> => {
