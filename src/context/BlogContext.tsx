...
export const BlogProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // 4. Hooks
  const { loading, generateOutlineWrapper, generatePostWrapper, generateSocialWrapper } =
    useBlogGenerator();
  const { suggestions, loadingTrends, refreshTrends } = useTrendingTopics(selectedModel);

  // 5. History Hook
  const { history, isHistoryOpen, setIsHistoryOpen } = useHistory(
    finalPost,
    finalPostEn,
    outline,
    creationId,
  ...
  const onGenerateFullPost = useCallback(async () => {
    ...
    try {
      // 1. Korean
      const postData = await generatePostWrapper(
        outline,
        selectedTone,
        selectedImageStyle,
        sourceFiles,
      ...
      let postDataEn = null;
      if (isDualMode) {
        postDataEn = await generatePostWrapper(
          outline,
          selectedTone,
          selectedImageStyle,
          sourceFiles,
      ...
      const titlePattern = new RegExp(
        `^${mainTitle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`,
        'i',
      );
      if (
        finalContent.match(titlePattern) ||
        finalContent.startsWith('Title:') ||
